...
uint32_t roundto(uint32_t r,uint32_t v){
	return (v/r)*r;
}
uint32_t FLASH_READ_64(uint32_t offset,uint64_t *buf,size_t len)
{
    if(offset+len > EEPROM_SIZE) return -1;
	if(len > EEPROM_PAGE_SIZE) return -2;
...
{
    if(off+len > EEPROM_SIZE) return -1;
	if(len > EEPROM_PAGE_SIZE) return -2;
...
uint32_t clear_page(uint64_t save[EEPROM_PAGE_SIZE],uint64_t addr){
